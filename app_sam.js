...
Note there are a few genres that are unlikley to have any events and I have determined should be merged under a "other"
category such as: Chanson Francaise, Children's Music,Holiday,Medieval/Renaissance, Other, Undefined.
*/
let layers = {
...
/*----------------------------------------------------------------------
NEW CHANGES: Overlapping Marker Spiderfier code
----------------------------------------------------------------------*/ 
...
/*------------------------------------------------------------
NEW CHANGES: Added select all and deselect all buttons to the map
------------------------------------------------------------*/
// Save L.control to the variable controlPanel
let controlPanel = L.control.layers(null, overlays).addTo(map);

// Add the "Select All" button to the layers control
var selectAll = L.DomUtil.create('button', 'select-all-button');
selectAll.innerHTML = 'Select All';
// add a event listener that checks if any layer is not selected and then selects it
selectAll.addEventListener('click', function() {
  Object.values(controlPanel._layers).forEach(function(layer) {
...
// Add the "Deselect All" button to the layers control
var deselectAll = L.DomUtil.create('button', 'deselect-all-button');
deselectAll.innerHTML = 'Deselect All';
// add a event listener that checks if any layer is selected and then deselects it
...
// Create a container for the button and append selectAll and deselectAll under it.
var Container = L.DomUtil.create('div', 'buttons-container');
Container.appendChild(selectAll);
Container.appendChild(deselectAll);

// Add the container to the layers control
controlPanel.getContainer().appendChild(Container);
...
/* ----------------------------------------------------------------------
Initialize a dictionary that contains icons for each layer group. 
---------------------------------------------------------------------- */
...
let icons = {
    ...
    //NEW CHANGES: reggae color is changed here and in the style.css file (lightgray)
    Reggae: L.AwesomeMarkers.icon({
        markerColor: icon_colors[18],
    }),
  ...
  d3.json(url_venues).then(function(venueRes) {
    ...
    d3.json(url_genres).then(function(genreRes) {
        ...
        for (let i = 0; i < doubleJoinedRes.length; i++) {
                ...
                /* -------------------------------------------------------
                Create the marker for doubleJoinedRes[i]
                ------------------------------------------------------- */
                ...
                //NEW CHANGES: newMarker gets added to the oms layer.
                ...
                /*###################################################
                IMPORTANT FOR SENDING INFO ABOUT THE SELECTED MARKER TO THE OTHER OBJECTS IN THE DASHBOARD
                ###################################################*/
